...
import org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.date_service.DateService;
import org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.greetings_service.Greeter;
...
import com.sitechasia.osgi.api_bridge.ApiBridge;
...
public class Main {
	...
	public void run(String[] args) {
		...
		Launcher launch = retrieveDaLauncherFactory().createLauncher();
		launch.launch();

		ServiceReference greeterServiceReference = launch
				.getOsgiFramework()
				.getBundleContext()
				.getServiceReference(
						"org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.greetings_service.Greeter");

		Object greeterServiceReferenceObject = launch.getOsgiFramework()
				.getBundleContext().getService(greeterServiceReference);

		ApiBridge apiBridge = ApiBridge.getApiBridge();

		Greeter greeter = (Greeter) apiBridge
				.bridge(greeterServiceReferenceObject,
						"org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.greetings_service");

		greeter.sayHi();

		//

		ServiceReference dateServiceReference = launch
				.getOsgiFramework()
				.getBundleContext()
				.getServiceReference(
						"org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.date_service.DateService");

		Object dateServiceReferenceObject = launch.getOsgiFramework()
				.getBundleContext().getService(dateServiceReference);

		DateService dateService = (DateService) apiBridge
				.bridge(dateServiceReferenceObject,
						"org.dynamicjava.examples.osgi.da_launcher.spring_dm_example.date_service");

		System.out.println(dateService.getCurrentDateFormatted());
	...
	protected LauncherFactory retrieveDaLauncherFactory() {
		try {
			...
			if (LauncherFactory.class.equals(launcherFactoryClassName)) {
				...
				Class<?> launcherFactoryClass = generateExtClassLoader()
						.loadClass(launcherFactoryClassName);

				try {
					Constructor<?> constructor = launcherFactoryClass
							.getConstructor(LauncherSettings.class);
					return (LauncherFactory) constructor
							.newInstance(getLauncherSettings());
				} catch (Throwable ex) {
			...
			throw new LauncherException(String.format(
					"Failed to load LauncherFactory: %s", ex.getMessage()), ex);
		}
	}
	...
	protected String retrieveLauncherFactoryClassName() throws Exception {
		File daLauncherExtConfigFile = new File(new File(getDaLauncerHomeDir(),
				InternalLauncherConstants.Directories.CONFIGURATIONS_DIR)
				.getAbsolutePath(),
		...
		if (daLauncherExtConfigFile.exists()) {
			...
			daLauncherExtProeprties.load(new FileInputStream(
					daLauncherExtConfigFile));

			String launcherFactoryClassName = daLauncherExtProeprties
					.getProperty(InternalLauncherConstants.DaLauncherExt.LAUNCHER_FACTORY);
	...
	protected ClassLoader generateExtClassLoader() {
		File[] files = new File(getDaLauncerHomeDir(),
				InternalLauncherConstants.Directories.LIB_DIR).listFiles();
		...
		return new URLClassLoader(jarUrls.toArray(new URL[0]), Thread
	...
	protected String locateDaLauncherHomeDir() {
		...
		while (true) {
			...
			if (files.contains("bundles") && files.contains("lib")
			...
			if (currentCheckedPath.equals(lastCheckedPath)) {
				throw new LauncherException(
						"DA-Launcher home directory can not be located.");
			}
