			...
			<table class="detailmain" cellspacing="0" cellpadding="0">
				<tr>
					<th class="pt" align="left" height="40">
						...
						<font color="red"> <b>拉路信息管理</b> </font> 编号：
							...
							<TBODY>
								<TR>
									...
									<TD width="70%">
										<%
											if ("1".equals(status)) {
												taskStatus = DispatchConstants.ddCutofftitle_new;
										%>
										 <input type="radio" name="transitionFlag" value="" checked >发送                                      
										<%}else if("2".equals(status)){
	                                            taskStatus = DispatchConstants.ddCutofftitle_approve;
                                        %><input type="radio" name="transitionFlag" value="1" checked>审核通过
                                          <input type="radio" name="transitionFlag" value="-1">审核驳回
                                        <%}else if("3".equals(status)){
	                                            taskStatus = DispatchConstants.ddCutofftitle_approve_YES;
                                        %><input id type="radio" name="transitionFlag" value="" checked>发布
                                        <%}else if("1m".equals(status)){
	                                            taskStatus = DispatchConstants.ddCutofftitle_approve_NO;
                                        %><input id type="radio" name="transitionFlag" value="" checked>流转到审核

                                        <%}else if("4".equals(status)){
	                                            taskStatus = DispatchConstants.ddCutofftitle_approve;
                                        %><input type="radio" name="transitionFlag" value="" checked>任务下发                                     
                                        <%}%>
									</td>
								</TR>
								<TR>
