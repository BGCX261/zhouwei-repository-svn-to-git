...
dhtmlXGridCellObject.prototype.setCValue = function(val,val2){	   
   // alert('setCValue');
	...
	/**
	 * 以下是扩展的,实现在多选框改变时,改变数据行的状态

	 */	
	
	rowid = this.cell.parentNode.idd;
	cellIndex = this.cell._cellIndex;
	colType = this.grid.getColType(cellIndex);
	//对于辅助选择框,不参与行状态的改变
	if (this.grid.isSelCheckBox==1 && cellIndex==1){
		//单击辅助选择框时,使当前行反显
	    if (this.grid.isDataLoaded == true) {
	...
	if ((this.grid.getColumnCount() - 1) == cellIndex) {
		return true;
	}
	if (this.grid.isDataLoaded == true && colType.indexOf('check') >= 0 ){		
		//获取数据行的状态

		rowStatusVal = this.grid.cells(rowid,this.grid.getColumnCount() - 1).getValue();
		
		//改变行的状态,对于数据库已有的数据
		if (rowStatusVal==1 || rowStatusVal==2) {
			//先获取当前行的主键值							   
			var privalue =  this.grid.getPriColVal(rowid);			
			rowStatusVal = this.grid.changeExRowStatus(rowid,cellIndex,rowStatusVal,privalue);
			
		}
		//改变行的状态,对于新行
		if (rowStatusVal == 4 || rowStatusVal == 3){								
		   rowStatusVal = this.grid.changeRowStatus(rowid,cellIndex,rowStatusVal);
		}	
	}   
}   
