/**
 * 这是一个日期时间的外挂工具tag,提供了文本框和日期控件组合及单独的日期控件. 此时需要将赋值的文本框参数传入.
 */
...
public class JHopDateTag extends JHopBaseTag {
	...
	public String getDivHtml() {
		...
		if (getGenText().equals("no")) {
			...
			htmlStr.append(" name=\"ss\" id='"
					+ "' border=\"0\" style=\"cursor:hand\"");
			htmlStr.append(" onclick=\"return showCalendar('"
					+ this.getEventStrId() + "','" + "'" + this.dateFormat
					+ "'" + ");\"");
			...
			if(showSecond)
			{
				htmlStr.append("<input type='text'" + " name='" + this.name + "_1' "
						+ " id='" + this.getEventStrId() + "_1' value='" 
						+ strValue+ "'");
				...
				htmlStr.append(" name=\"ss\" "
						+ "' border=\"0\" style=\"cursor:hand\" ");
				htmlStr.append("  onclick=\"return showCalendar('"
						+ this.getEventStrId() + "_1', " + "'" + this.dateFormat
						+ "'" + ");\" ");
				htmlStr.append("/>");
			}
			else
