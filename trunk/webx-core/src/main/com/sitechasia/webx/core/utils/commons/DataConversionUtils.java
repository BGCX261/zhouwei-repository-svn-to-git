...
public final class DataConversionUtils {
	 ...
	 * </pre>
	 */
	public static <T> void beans2excelFile(Map<String, String> titlebeanNameMap, List<T> beans, String excelFilePath)
			throws FrameworkException {
	 ...
	 * </pre>
	 */
	public static <T> List<T> excelFile2beans(List<String> beanAttrNames, Class<T> beanClass, String excelFilePath)
			throws FrameworkException {
	 ...
	 * </pre>
	 */
	public static <T> void beans2pdfFile(Map<String, String> titlebeanNameMap, List<T> beans, String pdfFilePath)
			throws FrameworkException {
		if (titlebeanNameMap == null || titlebeanNameMap.size() == 0) {
	...
	private static <T> void buildExcelDocument(HSSFWorkbook wb, Map<String, String> titlebeanNameMap, List<T> beans)
			throws FrameworkException {
		try {
			// 解析表单标题bean属性对应map
			// 保持两者一一对应,并且顺序一致
			...
			// 解析每一个bean
			// 每一个bean作为sheet的一个row
			// 每个bean属性值作为row的一个cell值
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static <T> List<T> parseExcelDocument(HSSFWorkbook wb, List<String> beanAttrNames, Class<T> beanClass)
			throws FrameworkException {
		...
		try {
			...
			// 每一个sheet的一个row作为一个bean对象
			// 每个row的一个cell值作为bean的对应属性值
			// 从序号为1的row开始解析,序号为0的row是文本标题
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static <T> void buildPdfDocument(Document document, Map<String, String> titlebeanNameMap, List<T> beans)
			throws FrameworkException {
		if (document == null) {
