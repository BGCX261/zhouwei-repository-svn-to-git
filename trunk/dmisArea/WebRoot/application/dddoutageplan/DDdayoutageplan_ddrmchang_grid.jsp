...
<html>
	...
	<%
	    ...
	    String url = "/ddDoutageplan.do?method=modechangeGridQuery&parentid="+parentid;
	...
	%>
	<%-- 创建弹出窗口，并传递参数 --%>
	<jhop:grid id="mygrid" busName="DdDoutageplanDto1"
		subName="128197051371530608" pageItems="30" listType="<%=listtype %>"
		listKind="<%=listkind %>" divHeight="150" isCheckBox="0" isTotalRow="0"
		listBox="fstationname#108^showStation"
		refColVal="zddfsmodechangelist4.fdayplanno^${parentid}"
		entityName="DdfsModechangelist"
		queryPath="<%=url %>">
   ...
   function setParentId(){
        if(mygrid){
            //alert('zddfsmodechangelist4.fdayplanno^<%=parentid%>');
			mygrid.setRefColVal('zddfsmodechangelist4.fdayplanno^<%=parentid%>');
			clearInterval(si);
		}
   }  
   ...
   //弹出对话框
			...
			function showStation(){
			    var  impo = new importDatasObj('/application/stdstation/showstdstation.jsp',{'id':'fstationid','stationname':'fstationname'},'mygrid');  
				impo.height = 500;
                impo.width = 400;
