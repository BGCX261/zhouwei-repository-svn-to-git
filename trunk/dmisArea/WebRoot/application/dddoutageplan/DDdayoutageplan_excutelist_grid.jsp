...
<% 
	 ...
	 */
	  ...
	  String url = "/ddDoutageplan.do?method=executeGridQuery&parentid="+parentid;
	  String refcol = "zdddayplanexcutelist1.fdayplanno^"+parentid;
%>

<%-- 创建弹出窗口，并传递参数 --%>
<%if(edit.equals("false")&&(delete.equals("false"))){ %>
<jhop:grid 	id="mygrid"
      busName="DdDoutageplanDto2"
      subName="128197172681541976"
      pageItems="30"
      listType="2"
      listKind="106"
      divHeight="150"
      ...
      isTotalRow="0" 
      refColVal ="<%=refcol %>"
      entityName="DdDayplanexcutelist"
      queryPath="<%=url %>">
</jhop:grid>
<%}else{ %>
<jhop:grid 	id="mygrid"
      ...
      listKind="104"
      divHeight="150"
      listBox="freturntime#5,freccmder#108^showfreccmder,freturnner#108^showfreturnner,fsendcmdtime#5,fspresponser#108^showGGre,fsendcmder#108^showfsendcmder,faccepter#108^showfaccepter"
      isCheckBox="0" 
      isTotalRow="0" 
      refColVal ="zdddayplanexcutelist1.fdayplanno^${parentid}"
      entityName="DdDayplanexcutelist"
      queryPath="/ddDoutageplan.do?method=executeGridQuery&parentid=${parentid}">
</jhop:grid>
<%} %>
<script>
...
<script>
   ...
   function setParentId(){
     var  refColVal ="<%=refcol %>"
        if(mygrid){
			mygrid.setRefColVal(refColVal);
			clearInterval(si);
		}
   }
   //接令人

   function  showfreccmder(){
      var  impo = new importDatasObj('/application/ggperson/GgPerson_show_grid.jsp',{'name':'freccmder'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
  ...
  function  showfreturnner(){
      var  impo = new importDatasObj('/application/ggperson/GgPerson_show_grid.jsp',{'name':'freturnner'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  ...
  function showGGre(){
  		   var  impo = new importDatasObj('/application/ggrelperson/GgRelperson_show_grid.jsp',{'name':'fspresponser'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  }
  ...
  function showfsendcmder(){
  		 var  impo = new importDatasObj('/application/ggperson/GgPerson_dispatch_grid.jsp',{'name':'fsendcmder'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  }
  ...
  //收令人 调度员收令
    ...
    function showfaccepter(){
  		 var  impo = new importDatasObj('/application/ggperson/GgPerson_dispatch_grid.jsp',{'name':'faccepter'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  }
		  ...
		  //把当前单元格的rowid和index存在变量里tem_rid,tem_ind
			...
			{       
					...
					{	
					    //显示日期控件
						temp_vale=null;
						Calendar2.Show(tempcel,true,'','yyyy-mm-dd hh:mm:ss','*');
					}
					if(state==2)
					{
					   //结束后把值保存
