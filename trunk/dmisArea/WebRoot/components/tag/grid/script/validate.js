...
checkObject.prototype.getValidateByName=function(){
 	try{
      ...
      //保存方法
    function saveDTOS(url,mygrid,validateflag,xmlpath,validatename,getAlldataFalg){
      	//获得xml
      	//alert(mygrid);
      	var mess ="是否要保存";
      	if(!confirm(mess))
      	  return;
      	 var dataxml;
      	if(getAlldataFalg&&getAlldataFalg==true){
      		//alert(getAlldataFalg);
      		dataXml = mygrid.serializeGridOfAll('3');
      	}else{
      	  	dataXml = mygrid.serializeGrid();
      	}
  ...
  function tanHead(head){
   ...
   function deleteDTOS(url,mygrid,idcol,vercol){
      alert(vercol);
	  ...
	  for (var i = 0;i<rows.length;i++){
	  	var  _id = mygrid.getValByRC(rows[i],idcol);
	  	alert(_id.length);
	  	if(_id&&_id!=null&&_id.length>0){
		  	if(i!=rowns.length-1){
	        	ids = ids +mygrid.getValByRC(rows[i],idcol)+";";
	        	version = version+mygrid.getValByRC(rows[i],vercol)+";";
  	   	...
  	   	if(ids!="undefined"&&ids.length>0){
  	   		var pars = 'ids='+ids+'&version='+version;
		    var myAjax = new Ajax.Request(encodeURI(url),{method: 'post', parameters: pars,  onComplete: showResponse});
		     function showResponse(originalRequest)
	       {
