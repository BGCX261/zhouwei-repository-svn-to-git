...
// var start = new Date().getTime();
 ...
 //alert(new Date().getTime()-start);
  ...
  /*
    @dwrmethod:call dwr method for exmple:Remote.getData
    @para:call dwr para
    @afterCall
    : callback
 */
 dhtmlXTreeObject.prototype.loadDWR=function(dwrmethod,para,afterCall){
 this.xmlstate=1;
 //alert(para);
  var parentobj = this;
    var dwrcopy = function(da){
        //alert(da);
        parentobj.loadXMLString(da);
    };
 	var executexmlstr = this.XMLsource+"('"+para+"',dwrcopy)";
 	//alert(executexmlstr);
 	//alert(executexmlstr);
 	eval(executexmlstr);
 //this.XMLLoader.loadXML(file);
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
{
 ...
 if((dhtmlObject.waitUpdateXML)&&(dhtmlObject._globalIdStorageFind(cId)))
 ...
 else
{
 ...
 var parentobj = this;
    var dwrcopy2 = function(da){
        var start = new Date().getTime();
        parentobj.loadXMLString(da);
        var end  = new Date().getTime();
        alert(end+"create node :"+(end-start));
    };
    var treename = this.getUserData(id,"treename");
    var text = this.getItemText(id);
    var nodisname = this.getUserData(id,"nodedisname");
    var userdata = this.getUserData(id,"userdata");
    var param = id+'|'+text+'|'+treename+'|'+nodisname+'|'+userdata;
 	var executexmlstr = this.XMLsource+"('"+param+"',dwrcopy2)";
 	//alert(executexmlstr);
 	eval(executexmlstr);
 	
 }
 else
 if(this.xmlalb=="action"){
	//alert(src);
	// alert(treename);
	 var text = this.getItemText(id);
     var userdata = this.getUserData(id,"info");
     var url = encodeURI(src+getUrlSymbol(src)+"uid="+sn+"&id="+id+"&info="+userdata+"&text="+text);
	 this.loadXML(url);
 }
 else
...
//设置图片路径 2007-2-1
 dhtmlXTreeObject.prototype.setLoadingImg=function(imgpath){
 	this.loadimgpath = imgpath;
 }
