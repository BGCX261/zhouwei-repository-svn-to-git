...
/*MAIN OBJECTS*/
   var isDragSelect = true;//用于控制鼠标拖动是对内容的选择
...
/*PROPERTIES*/
   ...
   //过滤行的图片信息 
   this.imgForFilterRowno="";
   this.imgForFilterCH="";
   //辅助选择列的标题图片
   this.imgForSelectAll="";
   this.imgForUNSelectAll="";
   ...
   this.contextPath = "";
   //列名
   this.colsName = new Array(0);
   //列名的备份,用于保存初始化时的列的顺序


   this.colsNameBak = new Array(0);
   //初次加载的数据,用于可维护列表
   ...
   this.gridObjName = "";
   //关联关系类名
   this.refClassName = new Array(0);
   //关联关系属性名   
   this.refName = new Array(0);
   //对应列是否是主键
   this.colIsPrimary = new Array(0);
   //对应的列是否可编辑


   this.colIsUpdate = new Array(0);
   //最后一个可编辑的列号


   this.lastEditColNum = 0;
   
   //对应的列是否显示
   this.colIsVisible = new Array(0);
   //列的hibernate数据类型
   this.hibdatatype = new Array(0);
   //过滤模式
   this.filterMode = 0;
   //过滤规则
   this.filterRules = new Array(0);
   //过滤列中的数据


   this.filterData = new Array(0);
   //服务端计算的指定列的统计值


   this.colTotalValue = new Array(0);
   //下拉列表框类型


   this.listBox = new Array(0);
   //下拉列表框数据源
   this.listBoxSrc = new Array(0);
   //查询界面中所设定的查询条件
   ...
   this.colValByListInfo = new Array(0);
   //总单的主键名、主键值、列索引号


   this.refColVal = "";
   //列的初始值,同时用于判断是否为空
   this.initdata = new Array(0);
   //单击列标题时的排序信息
   ...
   this.colsDisplayFormat = new Array(0);
   //业务层传递的查询
   this.extWheres = "";
   //业务层传递的排序
   this.extOrder = "";
   ...
   //临时变量，因为拖动列之后，竟然执行了两次afterDragCol,而第二次才是真实的,使用该变量控制
   ...
   this.initRows = 0;
   this.initDataForRows = new Array(0);
   //是否已经装载数据
   this.isDataLoaded = false;
   //操作列信(息列名^列值^type^url或列名^列值^type^function)
   this.opcolinfo = new Array(0);
   //是否可编辑    
   this.updateable = -1;
   //单击时，调用了多次showDialogByCondition,而且每次所传的值不同，只有第一次是正确的
   this.isExcuteShowDialogByCondition = false;
   /*EVENTS*/
      ...
      */
        this._doContClick=function(ev){
                     ...
                     var el = this.getFirstParentOfType(_isIE?ev.srcElement:ev.target,"TD");
					        ...
					        if (el.parentNode.idd == undefined){
								isDragSelect = false;
								//alert(el.parentNode.idd);
								//this.test2();
							}
      ...
      */
      this.doClick = function(el,fl,selMethod){		  
                  ...
                  if(selMethod!=1)
					 ...
					 //alert(this.row.idd+'   '+this.cell.cellIndex);
					 var colIndex = this.cell.cellIndex;
					 /*for (i=0;i<this.cell.cellIndex;i++){
						 if (this.colIsVisible[i] == 0) {
							 colIndex = colIndex + 1;
					     }
				     }
					 */
					 //alert(this.row.idd+'   '+colIndex);
					 //弹出之前，将辅助选择框置为选择状态
					 /*if (this.colIsVisible[1] == 1){
						this.cells(this.row.idd,1).setValue(1);
					 }
					 */
					 //在操作列上弹出对话框
					 //this.popUpForOp(this.row.idd,colIndex);
					 
					 this.showDialogByCondition(this.row.idd,colIndex);
                  }
      /**
      ...
      this.editCell = function(){
                     ...
                     if(this.editor!=null){
                           ...
                           if(this.onEditCell(0,this.row.idd,this.cell._cellIndex)!=false){
                              ...
                              this.editor.edit();	
							  //通过其它控件编辑指定的单元格
							  this.editCellByOtherObj(this.row.idd,this.cell._cellIndex);
      ...
      */
      this.doKey =   function(ev){
                     ...
                     if(this.isActive==false){
						...
						/**
						 *  手工录入列表
						 * 如果时最后一个可录入的单元格，则插入一新行,否则将焦点定位到本行的下一个可编辑的单元格
						 */
						if (ev.keyCode == 13 && this.gridKind == 104 && this.gridType == 2){
						   var isCheckSucceed;
						   var colval=this.cells(this.row.idd,this.cell._cellIndex).getValue();	
						   if (colval.length > 0){							   
								//进行有效性检查


							   isCheckSucceed = this.checkColValue(this.row.idd,this.cell._cellIndex,colval);
							   if (isCheckSucceed == false) {								  
								   return false;
							   }
						   }
							var rowNo = this.getRowsNum() + 1;
							//获取数据行的状态
							...
							//改变行的状态,对于新行
							if (statusColVal == 4 || statusColVal == 3){
							   statusColVal = this.changeRowStatus(this.row.rowIndex,this.cell._cellIndex,statusColVal);
							}
							//改变行的状态,对于数据库已有的数据
							if (statusColVal == 1 || statusColVal == 2){	
							   this.editStop();								
							   //先获取当前行的主键值							   
							   var privalue =  this.getPriColVal(this.row.rowIndex);
							   ...
							   statusColVal = this.changeExRowStatus(this.row.rowIndex,this.cell._cellIndex,statusColVal,privalue);
							   
							   return true;
							}
													
							//如果当前回车的行是新增的尚未填写内容的行,则不插入新行,将焦点定位到本行的下一个可编辑的单元格
							if ((statusColVal == 4 ) && (this.lastEditColNum == this.cell._cellIndex)) {
								//新增数据行


							    //this.initNewRow(rowNo);  作废该方法,因为该控件存在插入行时的不足
								this.addNewRow();
							    return true;						   
							}
							
							this.setFocus(this.row.rowIndex,this.cell._cellIndex);							   
							return true;
						}
                        ...
                        if(ev.keyCode==13 && !ev.ctrlKey && !ev.shiftKey){
						   ...
						   if ( this.row.idd == 0 ){
							   var colval;
							   var isCheckSucceed;
							   //alert(this.cell._cellIndex+'    '+this.getColType(this.cell._cellIndex));
							   colval=this.cells(this.row.idd,this.cell._cellIndex).getValue();
							   ...
							   if (isCheckSucceed == false) {
							      ...
							      //调用过滤方法,提交请求，刷新数据  this.queryPath
								  
								  this.callFilter(colval,this.cell._cellIndex);					 
							   }							   
   ...
   */
   this.sortField = function(ind,repeatFl){	              
                  ...
                  if(el.tagName == "TH" && (this.fldSort.length-1)>=el._cellIndex && this.fldSort[el._cellIndex]!='na'){
				   //单击行号列的列标题时,隐藏或显示过滤行					
				   if ((ind == 0) && (this.isFilter == 1)){						 
						 this.changeFilterRow();
						 return true;
				   }
				   //单击辅助选择列的列标题时,全选或撤消全选


				   if ((ind == 1 ) && (this.isSelCheckBox == 1)) {
						 var oldHeader = this.getHeaderCol(1);
						 var indexsubstr = oldHeader.indexOf("retract.gif");				 
						 if (indexsubstr > 0){
				   ...
				   if (this.gridType == 3 && this.gridKind == 101){
					   this.callServerSort(ind);
					   return true;
				   }
					  //this.entBox.fieldstosort!="" &&
...
//#}
   ...
   */
   dhtmlXGridObject.prototype._addRow   =    function(new_id,text,ind){
	              //alert('_addRow1::::   '+'new_id:'+new_id+'  '+text+'   ind:'+ind);
                  var row = this._fillRow(this._prepareRow(new_id),text);
				  //alert('_addRow2::::   '+'ind:'+ind+"   this.rowsCol.length:"+this.rowsCol.length+'  '+this.rowsCol.length+this.rowsBuffer[0].length);
...
dhtmlXGridObject.prototype.setColHidden=function(list){
   ...
   for (var i=0; i<this._ivizcol.length; i++)
      ...
      *   @type: private
      */
dhtmlXGridObject.prototype._fixHiddenRowsAll=function(pb,ind,state){
        var z=pb.rows.length;
        for (var i=0; i<z; i++){
            pb.rows[i].cells[ind].style.display=state;
		}
  }
/**
*   @desc: hide column
...
dhtmlXGridObject.prototype._hideShowColumn=function(ind,state){
    ...
    if (state=="none"){
        ...
        this._fixHiddenRowsAll(this.obj,ind,"none");
		if (this._fixHiddenRowsAllTG) this._fixHiddenRowsAllTG(ind,"none");
		if (this.ftr)
			this._fixHiddenRowsAll(this.ftr.childNodes[0],ind,"none");
        if (_isOpera||_isKHTML||(_isFF&&_FFrv<1.8))
			this._fixHiddenRowsAll(this.hdr,ind,"none");
        ...
        var zrow=this.hdr.rows[0].cells[ind];
              if (_isOpera||_isKHTML||(_isFF&&_FFrv<1.8))
				 this._fixHiddenRowsAll(this.hdr,ind,"");
			...
			if (this.ftr)
             ...
             this._fixHiddenRowsAll(this.obj,ind,"");
...
dhtmlXGridObject.prototype._createHRow = function(data,parent){
	if (!parent){
		...
		//由于目前列表控件中不支持删除统计行的功能,故在此修改,将t改为this.countDIV,以便调用
		//var t=document.createElement("TABLE");
		this.countDIV=document.createElement("TABLE");
		this.countDIV.cellPadding=this.countDIV.cellSpacing=0;
		if (!_isIE){
		   	this.countDIV.width="100%";
			this.countDIV.style.paddingRight="20px";
  		}
        this.countDIV.style.tableLayout = "fixed";
        this.countDIV.id="totalrow";
		z.appendChild(this.countDIV);
		this.countDIV.appendChild(document.createElement("TBODY"));
		this.ftr=parent=this.countDIV;

        var hdrRow =this.countDIV.insertRow(0);
        ...
        if (_isIE) {
		    //将绝对定位absolute改为相对定位relative
			hdrRow.style.position="relative";
...
*   @param: value - search needle
*   @param: c_ind - index of column for search (optional)
*   @edition: Professional
*   @returns: complex array
*   @type:  public
*/
dhtmlXGridObject.prototype.findCell = function(value,c_ind){
	var res=new Array();
