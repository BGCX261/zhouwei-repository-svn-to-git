...
/**
 *   是否可编辑
     该属性用于权限和列表控件的接口，具有控制列表是否可编辑的最高级别
     不使用tag或界面工具生成，只通过代码根据权限信息给其赋值

     权限系统返回 true/false  
     对应列表控件 1 / 0
     但若未获取权限信息，此属性值为:
 */
dhtmlXGridObject.prototype.setUpdateTable = function(updatetable){
	this.updateable = Number(updatetable);
}
/**
 * 设置列表类别
 */
dhtmlXGridObject.prototype.setGridClass = function(gridType,gridKind){
   ...
   //列表类型
   this.gridType = Number(gridType);
   //列表性质
   this.gridKind = Number(gridKind);
}
/**
 *  行号列中链接所调用的函数
 */
 ...
 *  判断是否点击了操作列
 */
dhtmlXGridObject.prototype.isShowDialog = function(rowid,colIndexid){
	var src;
	var url;
	var width;
	var height;
	var title;
	var scrolling;
	var newColIndexid = colIndexid;
	var priColVal;
	
	//根据隐藏列重新计算列的索引号
	for (idx=0;idx<this.colsName.length;idx++) {
		if (this.listBoxSrc[idx].split('@@@')[6] == this.colsName[idx]) {
           break;
        }
        if (this.colIsVisible[idx] == 0) {
			//如果在设置的操作列或伪列之前存在隐藏列,要将索引进行累加
           newColIndexid ++;
        }


		/*if (this.listBoxSrc[idx] == '' || typeof(this.listBoxSrc[idx]) == 'undefined' || this.listBoxSrc[idx] == null || this.listBoxSrc[idx] == 'null'){
			
		} else {
			if (!(this.listBoxSrc[idx].split('@@@')[6] == this.colsName[idx])) {
				//如果在设置的链接列之前存在隐藏列,要将索引进行累加
		        if (this.colIsVisible[idx] == 0) {
					newColIndexid ++;
				}
			} else {
	...
	if (this.colsName[newColIndexid].indexOf('opButton') >= 0 || this.hibdatatype[newColIndexid].toString().toLowerCase() == 'blob') {
		//alert('this.listBoxSrc[newColIndexid]:'+this.listBoxSrc[newColIndexid]+'\n\n'+this.colsName[newColIndexid]);
		if (this.listBoxSrc[newColIndexid].split('@@@')[6] == this.colsName[newColIndexid]) {
			
			if (Number(this.listBoxSrc[newColIndexid].split('@@@')[7]) == 1) {
				//自动弹出模态对话框
			    return 1+'^'+this.colsName[newColIndexid];
			} else {
				//运行在jsp中执行自定义的方法
				return 2+'^'+this.colsName[newColIndexid];
			}
		}
			
	}
	return 0+'^'+newColIndexid;
}
/**
 *  运行在jsp中执行自定义的方法
 */
dhtmlXGridObject.prototype.showDialogByCondition = function(rowid,colIndexid){	
  var retVal = this.isShowDialog(rowid,colIndexid).split('^');   
   switch (Number(retVal[0])) {
	   case 1:
	      
	      //按照tag中的设置弹出模态对话框
	      this.popUpForOp(rowid,colIndexid);
	      break;
	   case 2:	      
	      //调用jsp页面中用户自定义的方法
		  
	      if (typeof(exOperation) == 'function') {
			  exOperation(this,retVal[1]);
		  } else  if (typeof(parent.exOperation) == 'function') {
			  parent.exOperation(this,retVal[1]);
		  }
	      break;
	   case 0:	      
	      break;
 ...
 *  弹出模态对话框
 */
dhtmlXGridObject.prototype.showDialog = function(rowid,colIndexid){	
	var src;
	var url;
	var width;
	var height;
	...
	//单击弹出模态对话框	
	if (Number(this.listBox[newColIndexid]) == 8) {	
		src = this.listBoxSrc[newColIndexid].split('@@@');;
	    url = encodeURI(src[0]);
	    width = src[1];
	    height = src[2];
	    title = src[3];
	    ...
	    var tmp = priColVal.split("^");
	    priColVal = tmp[0]+'^'+tmp[1]+'^'+tmp[3];
		if (url.indexOf('?') > 0){
			url += '&primaryInfo='+priColVal;
		} else {
			url += '?primaryInfo='+priColVal;
		}
		
		//弹出之前，将辅助选择框置为选择状态
	    if (this.colIsVisible[1] == 1){
		   this.cells(rowid,1).setValue(1);
	    }
	    this.showModalDialogYY(url,width,height,param,title,scrolling);	
...
dhtmlXGridObject.prototype.popUpForOp = function(rowid,colIndexid){
	//alert(this.listBox+'\n\n'+this.listBoxSrc);
	//如果是过滤行,则不进行操作
	if (rowid == 0 && this.isFilter == 1) {		
		return;
	}
	if (colIndexid == 0) {
		return;
	}
	//alert(this.listBox+'   '+this.contextPath);
	var currurl = "";
	var found = false;
	var priColVal;
	var tmp;
	var newColIndexid = colIndexid;//根据隐藏列，重新计算单击列的索引
	
	//列名^列值^type^url
	var opcolname,opcolval,opcoltype,opcolurl;
	//弹出模态对话框
	if (this.showDialog(rowid,colIndexid) == 1) {
		return;
	}
	...
	for (idx=0;idx<this.colsName.length;idx++) {
		for (i=0;i<this.opcolinfo.length;i++){
		   //获取设置的链接列的列名
		   var arrInfoTmp = this.opcolinfo[i].split("^");
		   if (this.colsName[idx] == arrInfoTmp[0]){
			   break;
		   }
		}
		//如果在设置的链接列之前存在隐藏列,要将索引进行累加
	...
	for (i=0;i<this.opcolinfo.length;i++){
		...
		if (this.colsName[newColIndexid] == arrInfo[0]){
			opcolval = arrInfo[1];
			currurl = arrInfo[3];	
			found = true;
			break;
...
dhtmlXGridObject.prototype.setOpColInfoVal = function(){
	...
	for (i=0;i<this.opcolinfo.length;i++){		
		...
		if (!(arrOpInfo[0].indexOf('opButton_') >= 0 || arrOpInfo[0] == 'opButton' ) ) {
			continue;
		}
		for (j=0;j<this.colsName.length;j++){
			if (this.colsName[j].toLowerCase() == arrOpInfo[0].toLowerCase()){				    
				for (row=Number(srow);row<this.getRowsNum();row++){					
					if (!(arrOpInfo[0] == 'opButton' || arrOpInfo[0].indexOf('opButton_') >= 0)) {
	...
	if (rows <= 1 && this.isFilter == 1) {
		return;
	}
	//弹出模态对话框时
	for (colIndexid=0;colIndexid<this.colsName.length;colIndexid++){		
	    
		switch (Number(this.listBox[colIndexid])){
			case 1:
			   break;
			case 2:
			   break;
			...
			case 7:
			   break;
			case 8:
			   //不仅支持操作列、伪列，也支持blob字段
			   if (this.colsName[colIndexid] == 'opButton' || this.colsName[colIndexid].indexOf('opButton_') >= 0 || this.hibdatatype[colIndexid].toString().toLowerCase() == 'blob') { 					 
				 //弹出式的模态对话框,提取src中的操作列的显示值				 
				 src = this.listBoxSrc[colIndexid].split('@@@');	
				 if (src.length < 6 ) {
					 alert('tag中的设置有错!');
					 return;
				 }
				 if (!(src[5] == '' || src[5] == null || typeof(src[5]) == 'undefined' || src[5] == 'null')) {
					 for (row=1;row<=rows;row++) {
						 if (this.isItemExists(row)) {							
							this.cells(row,colIndexid).setValue(src[5]);
							this.cells(row,colIndexid).setTextColor('blue');
						 }
					 }
				 }
			   }
...
dhtmlXGridObject.prototype.setCellWidth = function(widthStr){
	...
	for (i=0;i<colWidths.length;i++){
		...
		/*if (i == (colWidths.length -1)){
			alert('colWidths['+i+']:'+colWidths[i]+'   colWidth:'+colWidth +'   newColWidthSum:'+newColWidthSum);
			if (newColWidthSum > 100) {
				if (Number(colWidth) > (Number(newColWidthSum) - 100)){
			        colWidth =Number(colWidth - (newColWidthSum - 100));
				} else if (Number(colWidth) < (Number(newColWidthSum) - 100)){
					colWidth = (newColWidthSum - 100) - colWidth;
				}
			}
...
dhtmlXGridObject.prototype.getSelectedRowsData = function (opType){
    ...
    if (Number(this.colIsVisible[1]) == 1){
		//使用复制选择框
		for (i=0;i<=rows;i++){			
			if (this.isItemExists(i) == false) { 
			    break;
			}
			if (Number(this.cells(i,1).getValue()) == 1) {
				row = i;
				break;
			}
		}
	...
	if (opType == 3){
		for (var i=1;i<this.getColumnCount();i++){
			if ( this.colsName[i] != "rowno" && this.colsName[i] != "selCheckBox"  && this.colsName[i].indexOf('opButton') < 0 && this.colsName[i] != "rowStatus" ) {
			   rowdata += this.colsName[i] + "^" + this.cells2(row,i).getValue()+"@@@";
			}
		}
		retvalue = rowdata.substr(0,rowdata.length - 3);	
	...
	if (opType == 4){
		if (this.colIsVisible[1] == 1) {
		   //使用复制选择框
		   for (i=1;i<=rows;i++){
			  if (this.isItemExists(i) == false) { break;}
			  if (Number(this.cells(i,1).getValue()) == 1) {				
				 num ++;
				 if (tmpAr.length == 0 || tmpAr == ""){
					 tmpAr = Number(i).toString();
				 } else {
					 tmpAr += ","+i;
				 }
			  }
		   }	
		   rownoForCH = tmpAr.split(",");
		} else {
			//使用反显选择
		   num = this.selectedRows.length;	
		   tmpAr = this.selectedRows;
		}
		if(num==0) return;
		//扫描选择的行
		for(var i=num-1;i>=0;i--){	
		   for (var j=0;j<this.getColumnCount();j++){
			   if ( this.colsName[j] != "rowno" && this.colsName[j] != "selCheckBox"  && this.colsName[j].indexOf('opButton') < 0  && this.colsName[j] != "rowStatus" ) {
				  if (this.colIsVisible[1] == 1){
					  rowdata += this.colsName[j] + "^" + this.cells(rownoForCH[i],j).getValue()+"@@@";
				  } else {
			          node = tmpAr[i];
					  rowdata += this.colsName[j] + "^" + this.cells2(node.idd,j).getValue()+"@@@";	
				  }
			   }
		   }
		   rowdata = rowdata.substr(0,rowdata.length - 3)+"###";
		}
		retvalue = rowdata.substr(0,rowdata.length - 3);
		return retvalue;
	}
		...
		for(var i=num-1;i>=0;i--){		                  
		   ...
		   for (var j=0;j<this.getColumnCount();j++){
			   if ( this.colIsPrimary[j] == 1 ) {
				  if (this.colIsVisible[1] == 1){
					  coldata += this.colsName[j] + "^" + this.cells(Number(rownoForCH[i]),j).getValue()+"@@@";	
				  } else {
					  node = tmpAr[i];
...
//dhtmlXGridObject.prototype.getAllRowsData
 
/**
 * 返回选择的行号,区分辅助选择框和反显选择
 */
dhtmlXGridObject.prototype.getSelectedRows = function(){
	var rowsNo;
	var tmpAr = "";
	var num=0;
	var rows = this.getRowsNum();  //总行数
	if (this.colIsVisible[1] == 1){
	   ...
	   if (num == 0) {
		  ...
		  rowsNo = this.selectedRows;
		  for (n=0;n<rowsNo.length;n++){
		  	 node = rowsNo[n];
		  	 if (tmpAr == "" || tmpAr.length == 0){
				 tmpAr = Number(node.idd).toString();
			 } else {
				 tmpAr += ","+Number(node.idd);
			 }
		  }
		  rowsNo = tmpAr.split(",");
	   }
 ...
 * 通过列名获取列值
 */
dhtmlXGridObject.prototype.getValueByName = function(colName){
	var colIndex = 0;
	var colVal;
	...
	if (colIndex ==0 ){ 
	   alert("列表中找不到'"+colName+"'的信息!");
	   return;
	}
	colVal = this.cells(row,colIndex).getValue();
	return colVal;
}
/**
 * 获取指定行中的指定列名的列值
 */
dhtmlXGridObject.prototype.getValByRC = function(rowid,colName){
    var colIndex = 0;
    var colVal;
    for (i=0;i<this.colsName.length;i++){
       if (this.colsName[i].toLowerCase() == colName.toLowerCase()){
           colIndex = i;
           break;
       }
    }
    if (colIndex == 0) {
       alert('未找到列名为('+colName+')的数据!');
       return "";
    }
    if (rowid > 0) {
       colVal = this.cells(rowid,colIndex).getValue();
    }
    return colVal;
}
/**
...
dhtmlXGridObject.prototype.setReferenceColVal = function(rowid){
   ...
   if (refStr == null || refStr == "" || refStr == 'null' || typeof(refStr) == 'undefined') {
	  return false;
   }
...
dhtmlXGridObject.prototype.addNewRow = function(){
	...
	if (rowid > 1) {
		...
		this.cells(Number(rowid),0).setValue(Number(rowid)+'^javascript:callLink();^_self');
	}
...
dhtmlXGridObject.prototype.insertRowBefore = function(rowid){	
	...
	if (this.colIsVisible[1] == 1) {
	   ...
	   this.addRow(newrowid,["1"],newrowindex);
	}
...
dhtmlXGridObject.prototype.setListBox = function(listBox,listBoxSrc){	   
   ...
   for (var i=0;i<arrlistbox.length;i++){	
       if (arrlistbox[i] == '' || typeof(arrlistbox[i]) == 'undefined' || arrlistbox[i] == null || arrlistbox[i] == 'null') {
	       this.listBox[i] = 0;	
	   } else {
		   this.listBox[i] = Number(arrlistbox[i]);	
	   }
 ...
 * begin_date#6^1@1@6,end_date#6^1@2@3
 * 返回本组中对应的列的
 */
dhtmlXGridObject.prototype.getDateGroup = function(rowid,colIndex){
	var dateGroupInfo = this.listBoxSrc[colIndex];
	var newDateGroupInfo,newDateGroupInfoPer;
	var dateGroupNo = 0;//日期组号
	var dateFlag = 0;  // 日期开始终止标准
	...
	if (dateGroupInfo == null || dateGroupInfo == '') {
		return '';
	}
	var dateGroupInfoPer = dateGroupInfo.split('@');
	if (dateGroupInfoPer.length < 2) {
		return '';
	}
	dateGroupNo = dateGroupInfoPer[0];
	dateFlag = dateGroupInfoPer[1];
	for (i=0;i<this.listBoxSrc.length;i++){
	   if (i == colIndex) { continue;}
	   if (this.listBox[i] == 6){
		   newDateGroupInfoPer = this.listBoxSrc[i].split('@');
		   if (newDateGroupInfoPer[0] == dateGroupNo){
			   val = this.cells(rowid,i).getValue();
			   val += '@'+newDateGroupInfoPer[2];
			   break;
		   }
	   }
...
dhtmlXGridObject.prototype.getPriColVal3 = function (rowid){
	 ...
	 for (var j=0;j<this.getColumnCount();j++){
		 if ( this.colIsPrimary[j] == 1 ) {
			 coldata += this.refName[j]+"^"+this.cells2(rowid,j).getValue()+"@@@";	
			// coldata += this.refName[j]+"^"+this.cells(rowid + 1,j).getValue()+"@@@";
...
dhtmlXGridObject.prototype.genFilterstr = function(colvalue,cellIndex){
	...
	if (this.filterMode == 2) {
		for(c=2;c<this.getColumnCount() - 1;c++){
			...
			//不依据特殊列进行查询
			if (colTmpName.indexOf('opButton') >= 0 || colTmpName == 'rowno' || colTmpName == 'selCheckBox' || colTmpName == 'rowStatus'){
				continue;
			}
...
dhtmlXGridObject.prototype.callFilter =  function(colvalue,cellIndex) {
	...
	//分页信息	
	this.pageno = 0; //为了每次分页都从第一页开始，否则报错
...
/**
 *  删除数据后,根据最后的查询条件、过滤条件、分页信息再次刷新列表
 */
dhtmlXGridObject.prototype.anewGridAfterDelete =  function() {
	//扫描最后的一个可编辑列
	var columnCount=0;
	var columnType;   
	var filterstr='';
	...
	var entityName="";	
	var colName="";
	var selectColNames = this.selectColNames;
	if (selectColNames == null || selectColNames == 'null' || selectColNames == 'undefined') {
		selectColNames = '';
    }
	//获取过滤参数
	if (this.isFilter == 1) {
	   columnCount = this.colsName.length;
	   for (cellIndex=columnCount - 1;cellIndex > 1;cellIndex--) {	
	      colName = this.colsName[cellIndex];
	      if (colName == 'rowStatus' || colName.indexOf('opButton') >= 0 || colName == 'selCheckBox' || colName == 'rowno'){
			  continue;
		  }
		 
		  columnType = this.getColType(cellIndex); 	
		 
		  if (columnType == 'ed' || columnType == 'price') {
			  colvalue = this.cells(0,cellIndex).getValue();
			  if (colvalue == null || colvalue == 'undefined' || colvalue.toString()._dhx_trim() == '' ) {
				  continue;
			  }
	...
	if ((this.refColVal == "" || this.refColVal == null || this.refColVal == 'null' || this.refColVal == 'undefined')){		
		
	} else {
		//salesId^string^1^639697^^asc
		var tmp = this.refColVal.split('^');
		if (wheres == null || wheres == ""){
			wheres = tmp[0]+'^'+'string'+'^'+'1'+'^'+tmp[1]+'^^asc'+'###'
		} else {
			wheres = tmp[0]+'^'+'string'+'^'+'1'+'^'+tmp[1]+'^^asc'+'@@@'+wheres
	...
	if (wheres == null || wheres == 'null' || wheres == 'undefined'){
		wheres = '';
	}
	//设置持久化类名称
	if (this.entityName == "" || this.entityName == null){			
	...
	//分页信息	
	this.pageno = 0; //为了每次分页都从第一页开始，否则报错
	var pageStr = this.genPageStr();
	if (pageStr == null || pageStr=='undefined'){
		pageStr = '';
	}
	//其它参数:合计信息,操作列信息,列表类型,列表性质...
	var otherparams = this.genOtherparams();
	...
	createGrid(entityName,0,"",pageStr,otherparams,wheres,filterstr,selectColNames);
}
/**
 * 执行服务端的排序
 */
dhtmlXGridObject.prototype.callServerSort = function(colind){
	...
	var entityName = "";
	var otherparams = "";
	//由于特殊列在数据库中是不存在的，就不参与排序了
	if (this.colsName[colind].indexOf('opButton_') >= 0 || this.colsName[colind] == ('rowStatus') ) {
		return;
	}
	if (this.sortonlycol == "" || this.sortonlycol == null || this.sortonlycol == "null" || this.sortonlycol.indexOf(this.colsName[colind] + "^") != 0){
...
dhtmlXGridObject.prototype.rowChanged = function(curRowId){
	...
	*/
	...
	if (typeof(rowChangedToChild) == 'function') {		
	   //alert('rowChangedToChild');
	   rowChangedToChild(priColVal,this.gridKind,rowData);
	   return true;
	} else if (typeof(parent.rowChangedToChild) == 'function') {
	   //alert('parent.rowChangedToChild');
	   parent.rowChangedToChild(priColVal,this.gridKind,rowData);
	   return true;
	}
}
 ...
 * 强制性改变列表的编辑属性
 */
dhtmlXGridObject.prototype.forceChangeEdit = function(){
	switch (this.updateable) {
		  case -1:
			 //默认
			 break;
		  case 0:
...
dhtmlXGridObject.prototype.afterLoad = function(){ 
   ...
   // }
	...
	//释放给用户使用，在装载完数据后进行相关的控制
	if (typeof(afterLoadForUser) == 'function') {
	   afterLoadForUser(this);
	}
	//双击数据行时，所调用的方法
	if ( this.getRowsNum() > 0 ) {		
	       this.setOnRowDblClickedHandler(this.exDoOnRowDblClicked);
	}
 ...
 *  双击数据行时，所调用的方法
 */
dhtmlXGridObject.prototype.exDoOnRowDblClicked = function(rowId){	
   if (typeof(doOnRowDblClickedForUser) == 'function') {
	   //在jsp页面中定义的方法
	   doOnRowDblClickedForUser(this,rowId);
   }
}
/**
 * 删除统计行
...
dhtmlXGridObject.prototype.genTotalRow = function(){
   ...
   for (i=1;i<this.colTotalValue.length;i++){
	   if (totalStr=="") {
	      ...
	      if (this.colTotalValue[i] == null || this.colTotalValue[i] == ''){
			  totalStr += ",";
			  continue;
		  }
...
/**
 * 对序列化的数据进行整理，按照参数删除xmlstr中的多余数据
 *  delFlag(也可以是以下各个参数值的组合  如：2,4):
 *     1  删除状态为1的数据
 *     2  删除状态为2的数据
 *     3  删除状态为3的数据
 *     4  删除状态为4的数据
 */ 
dhtmlXGridObject.prototype.adjustXMLData = function(xmldata,delFlag){
	var rowStatus;
	var rowNo;
	var xmldatatmp = "",oldxmldata="";
	var rowplace1,rowplace2;
	...
	for(row=sRow;row<=this.getRowsNum();row++){
		rowStatus = this.cells(row,this.getColumnCount() - 1).getValue();			
		if (!(delFlag.indexOf(rowStatus) >= 0)) {				
			splace = "<row "+"id='"+row+"'";
			eplace = 'row>';
			rowplace1 = newxmldata.indexOf(splace);
			rowplace2 = newxmldata.indexOf(eplace,rowplace1);			
...
/**
 * 序列化列表中的所有正常数据(不包含特殊列)
 */
dhtmlXGridObject.prototype.serializeGridOfAll = function(delFlag){	
   var xmlStr = "";
   var isSerialize;	
	//进行序列化时列的选择
	for (i=0;i<this.colsName.length;i++){
	...
	if (delFlag == 0) {
	   ...
	   //删除多余的数据
	   xmlStr = this.adjustXMLData(xmlStr,delFlag);
	}
	return xmlStr;
}
/**
...
dhtmlXGridObject.prototype.genComDateVal = function(rowid,colid) {
	...
	var windowUrl = this.contextPath + "/components/tag/grid/calendar/comQueryDate.jsp";
	//获取单元格
 ...
 *  弹出模态对话框,并居中显示
 */
dhtmlXGridObject.prototype.showModalDialogYY = function(url,width,height,setObj,title,scrolling){
        var top = (screen.height - height)/2;
        var left = (screen.width - width)/2;

		var frameWindowUrl = this.contextPath + "/common/generate_iframe.jsp?title="+title+"&scrolling="+scrolling;
		setObj.url = url;		
...
dhtmlXGridObject.prototype.editCellByOtherObj = function(rowid,colid){	
	...
	switch (Number(this.listBox[colid])){
		...
		case 8:
		   ...
		   alert('弹出包含列表的模态对话框');
		   //弹出包含列表的模态对话框		   
	       var src = this.listBoxSrc[colid].split('@@@');;
		   var url = src[0];
		   var width = src[1];
		   var height = src[2];
		   var title = src[3];
