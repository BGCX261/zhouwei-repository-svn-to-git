			...
			<table class="detailmain" cellspacing="0" cellpadding="0">
				<tr>
					<th class="pt" align="left" height="40">
						业务对象：
						<font color="red"> <b>日停电计划管理</b> </font> 编号：
						<font color="red"> <b><%=id%>
						</b> </font>
					</th>
				</tr>
							...
							<TBODY>
								<TR>
									...
									<TD width="70%">
										<%
											if ("1".equals(status)) {
												taskStatus = DispatchConstants.AREA_DdDoutageplan_NEW;
										%>
										 <input type="radio" name="transitionFlag" value="" checked >发送
										<%}else if("2".equals(status)){
	                                            taskStatus = DispatchConstants.DdDayPlan_WORKFLOW_AREA_TIDY;
                                        %><input type="radio" name="transitionFlag" value="1" checked >不需要其他调度审核
                                          <input type="radio" name="transitionFlag" value="2">需要其他调度审核
                                          <input type="radio" name="transitionFlag" value="3">驳回
                                        <%}else if("3".equals(status)){
                                           taskStatus = DispatchConstants.DdDayPlan_WORKFLOW_AREA_COURSIGNING;
                                         %>
                                           <input type="radio" name="transitionFlag" value="1" checked >会签通过
 										   <input type="radio" name="transitionFlag" value="-1">会签驳回
                                        <%
                                        }else if("4".equals(status)){
                                        	 taskStatus = DispatchConstants.DdDayPlan_WORKFLOW_AREA_APPROVE;
                                        %>
                                        	 <input type="radio" name="transitionFlag" value="1" checked >调度权限属于其他单位的计划
											 <input type="radio" name="transitionFlag" value="-1">调度权限属于本单位的计划
                                        <%
                                        }else if("5".equals(status)){

                                            taskStatus = DispatchConstants.AREA_DdDoutageplan_APPROVEED;
                                        %>
                                        	<input type="radio" name="transitionFlag" value="" checked>提交
                                        <%
                                         }else if("6".equals(status)){
                                           taskStatus = DispatchConstants.AREA_DdDoutageplan_DISPATCHED;
                                        %>
                                        	<input type="radio" name="transitionFlag" value="" checked>归档
                                         <%
                                         }else if("7".equals(status)){
                                           taskStatus = DispatchConstants.AREA_DdDoutageplan_PERFORMED;
                                        %><%if("客户中心进行日计划可否执行的批准操作".equals(taskName)){

                                         %><input type="radio" name="transitionFlag" value="1" checked>通过
                                           <input type="radio" name="transitionFlag" value="-1">驳回
                                         <%}else if("处理日计划，通知客户".equals(taskName)){

                                        %><input id type="radio" name="transitionFlag" value="" checked>发送通知
							   ...
							   <%if("2".equals(status)){%>
						          <TR>
						            ...
						            <TD width="70%">
						              <input type="checkbox" name="nextTaskRoles" value="城区供电公司通知客户专工">城区供电公司通知客户专工&nbsp;
						              <input type="checkbox" name="nextTaskRoles" value="城区供电公司客服中心"  >城区供电公司客服中心&nbsp;
						            </td>
						          </TR>
					          <%}%>
								<TR>
