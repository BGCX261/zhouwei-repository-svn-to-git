...
<html>
	...
	<body>
		...
		<%
				...
				request.setAttribute("extWheres","t.zddoperationnotebook4.fid='"+parentid+"'");
		%>
	...
	<%-- 创建弹出窗口，并传递参数 --%>
	<jhop:grid id="mygrid" pageId="standGridPage3"
		busName="DdClassondutylogDto3" subName="128199635774441464"
		pageItems="40" listType="2" listKind="104"
		entityName="DdOperationnotebook" isDisplaySet="1" isQuerySet="1"
		isPageSet="1" isOutputSet="1" isCheckBox="0"
		refColVal="zddoperationnotebook4.fid^${parentid}"
		listBox="focurtime#5^,fiswritelog#101^是@否,fiswriteincurlog#101^是@否"
		queryPath="/ddClassondutylog.do?method=standGridQuery3&extWheres=${extWheres}">
	</jhop:grid>
   ...
   function setParentId(){
        if(mygrid){
			...
			mygrid.setRefColVal('zddoperationnotebook4.fid^<%=parentid%>');
			clearInterval(si);
		}
   ...
   //交令人
  function  showfsummiter(){
      var  impo = new importDatasObj('/application/ggperson/GgPerson_show_grid.jsp',{'name':'fsummiter'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
  ...
  //下令人 调度员下令
  function showfgiver(){
  		 var  impo = new importDatasObj('/application/ggperson/GgPerson_dispatch_grid.jsp',{'name':'fgiver'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  }
  ...
  //收令人 调度员收令
    function showfaccepter(){
  		 var  impo = new importDatasObj('/application/ggperson/GgPerson_show_grid.jsp',{'name':'faccepter'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
           impo.showImportDatas();
  }
       ...
       // alert(headCols);
		  ...
		  function dateChanged(calendar) {
			  if(calendar.dateClicked){
				 ...
				 temp_vale = formatdd('yyyy-MM-dd hh:mm:ss',calendar.date);
