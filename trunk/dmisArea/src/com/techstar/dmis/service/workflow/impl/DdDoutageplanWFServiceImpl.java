...
// Source File Name:   DdDoutageplanWFServiceImpl.java
...
public class DdDoutageplanWFServiceImpl
    implements IDdDoutageplanWFService
{
        ...
        throws DataAccessException
    {
        try
        {
            ...
            if("-1".equals(transitionflag))
                workFlowDao.endTask(Long.parseLong(dto.getTaskInstanceId()), DispatchConstants.DdDayPlan_WORKFLOW_AREA_SENT_TRANSIT_THIS, dto.getUserId(), dto.getNextTaskRoles());
            else
            if("1".equals(transitionflag))
                workFlowDao.endTask(Long.parseLong(dto.getTaskInstanceId()), DispatchConstants.DdDayPlan_WORKFLOW_AREA_SENT_TRANSIT_OTHER, dto.getUserId(), dto.getNextTaskRoles());
            handleHistoryDao.saveOrUpdate(dto);
        }
        catch(Exception ex)
        {
        ...
        throws DataAccessException
    {
        ...
        {
            String transitionflag = dto.getTransitionFlag();
            if("2".equals(transitionflag))
                workFlowDao.endTask(Long.parseLong(dto.getTaskInstanceId()), DispatchConstants.DdDayPlan_WORKFLOW_AREA_ARRANGE_TRANSIT_NEED, dto.getUserId(), dto.getNextTaskRoles());
            else
            if("1".equals(transitionflag))
                workFlowDao.endTask(Long.parseLong(dto.getTaskInstanceId()), DispatchConstants.DdDayPlan_WORKFLOW_AREA_ARRANGE_TRANSIT_NO_NEED, dto.getUserId(), dto.getNextTaskRoles());
            else
            if("3".equals(transitionflag))
                workFlowDao.endTask(Long.parseLong(dto.getTaskInstanceId()), DispatchConstants.DdDayPlan_WORKFLOW_AREA_ARRANGE_TRANSIT_REBACK, dto.getUserId(), dto.getNextTaskRoles());
            handleHistoryDao.saveOrUpdate(dto);
        }
        catch(Exception ex)
        {
    ...
    public int ddDayPlanEnd(WorkflowHandleDto dto)
        throws DataAccessException
    {
        try
        {
