...
<%
    ...
    String listBox = "fprocesstime#5,fhandler#108^showfreccmder";
    
    String refColVal="zddaccidentbriefprocess1.faccidentid^"+parentid;

    String url = "/ddAccidentbrief.do?method=standGridQueryProcess&parentid="+parentid;
...
<jhop:grid id="mygrid" pageId="standGridPage1"
	busName="DdAccidentbriefProcessDto" subName="128208434031255332" pageItems="20"
	listType="2" listKind="105" isCheckBox="0" isTotalRow="0" listBox="<%=listBox%>"
	divHeight="700" entityName="DdAccidentbriefprocess"
	refColVal="<%=refColVal%>"
	queryPath="<%=url%>">
</jhop:grid>
<script>
  ...
  function setParentId(){
        ...
        if(mygrid){
			mygrid.setRefColVal('zddaccidentbriefprocess1.faccidentid^<%=parentid%>');
			clearInterval(si);
		}
   }  
</script>
<script>
   ...
   function  showfreccmder(){
      var  impo = new importDatasObj('/application/ggperson/GgPerson_show_grid.jsp',{'name':'fhandler'},'mygrid');  
	       impo.height = 500;
           impo.width = 600;
	 ...
	 function editHandler(state,rid, ind){
       ...
       // dealAlertMessage(headCols);
		  //把当前单元格的rowid和index存在变量里tem_rid,tem_ind
			...
			//dealAlertMessage("listbox="+listbox);
			if(listbox=="5")//日期类型
			{       
					//取得单元格对象
				   ...
				   //取得单元格对象
					...
					{   temp_vale=null;
					    ...
					    {   
					    	...
					    	//dealAlertMessage("content="+content)
					    	var  typ = (content[0].split("="))[1];//取得type
					    	//dealAlertMessage(typ);
					    	var  parent = (content[1].split("="))[1];//取得parentId
					    	//dealAlertMessage("ffff");
