...
| 2003-04-01 | added document arguments to MenuBar create and .          |
...
Menu.prototype.setChangeVisible=function(changfun){
	if(typeof(changfun)=="function")
		this.changeVi=changfun;
	else
		this.changeVi=eval(changfun);
}
...
Menu.prototype.toHtml = function () {
	...
	return  "<html><head>" +
			...
			"<table id='minnertable' cellspacing=\"0\">" +
...
Menu.prototype.show = function ( left, top, w, h ) {
	...
	h = h || Math.min( window.screen.height, this.getPreferredHeight() );
     //设置显示状态
    if(typeof(this.changeVi)=="function"){
    	 var d = this.getDocument();
    	 var  tableel = d.getElementById("minnertable");
    	 var  trs = tableel.getElementsByTagName("tr");
         for(var i=0;i<trs.length;i++){
            //alert(trs[i].id);
         	var h = this.changeVi(trs[i].id);
         	if(h){
         		trs[i].style.display="";
         	}else{
    ...
    var width = this.popup.document.body.scrollWidth;
    var height = this.popup.document.body.scrollHeight;
	this.popup.hide();
	this.popup.show( left, top, width, height);
...
MenuItem.prototype.toHtml = function () {
	...
	var key1 = this.pkey;
	return	"<tr " +((key1 != ""&& key1!=null)? " id=\"" + key1 + "\"" : "")+" "+
...
MenuBar.prototype.write = function (ele) {
	this._document = document;
	//this._document.write( this.toHtml() );
	ele.innerHTML = this.toHtml()
