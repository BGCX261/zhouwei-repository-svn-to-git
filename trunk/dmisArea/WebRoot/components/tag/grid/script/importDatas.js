	   	 ...
	   	 */
		function importDatasObj(url,relation,gridname,doc,title,param,scoll,height,width){
	   	...
	   	/*
	   	 * 通过传入的数据字段名称查找对应的列表字段名称或者文本域的名称
	   	 */
	   	...
	   	//
	   	importDatasObj.prototype.tranDatasToinput=function(data){
	   		// var aferTrandata ="";
	   		  try{
		   	      var datas = data.split("@@@");
		   		  for(var i=0;i<datas.length;i++){
		   		  	var field = datas[i].split("\^")[0];
		   		  	var value1 = datas[i].split("\^")[1];
		   		  	var tranfield = this.getTargetField(field);
		   		  	if(tranfield){
			   		  	var inpu=this.doc.getElementById(tranfield);
			   		  	inpu.value=	value1;
		   		  	}
		   		  }
	   	 ...
	   	 * @return  
	   	 */
	   	importDatasObj.prototype.inportdatastoinput=function(){
	   			var vReturnValue = showModalDialogYY(this.url,this.width,this.height,this.param,this.title,this.scoll);
	   	       if(vReturnValue){
	   	            if(this.relation)
		   	       		this.tranDatasToinput(vReturnValue);
	   	        } 
	   	}
