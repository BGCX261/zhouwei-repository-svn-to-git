			...
			<table class="detailmain" cellspacing="0" cellpadding="0">
				<tr>
					<th class="pt" align="left" height="40">
						...
						<font color="red"> <b>电网风险分析</b> </font> 编号：
							...
							<TBODY>
								<TR>
									...
									<TD width="70%">
										<%
											if ("1".equals(status)) {
												taskStatus = DispatchConstants.PowerRiskAnalysis_STATUS_NEW;
										%>
										 <input type="radio" name="transitionFlag" value="" checked >发送                                      
										<%}else if("2".equals(status)){
	                                            taskStatus = DispatchConstants.PowerRiskAnalysis_STATUS_APPROVE;
                                        %><input type="radio" name="transitionFlag" value="1" checked>审核通过
                                          <input type="radio" name="transitionFlag" value="-1">审核驳回
                                        <%}else if("3".equals(status)){
	                                            taskStatus = DispatchConstants.PowerRiskAnalysis_STATUS_APPROVE_YES;
                                        %><input id type="radio" name="transitionFlag" value="" checked>发布
                                        <%}else if("1m".equals(status)){
	                                            taskStatus = DispatchConstants.PowerRiskAnalysis_STATUS_APPROVE_NO;
